...
The proof generated by `decide` under the hood is equivalent to the following for our purposes:[^4]
...
It states that if evaluating the decision procedure[^5] for proposition `p` returns `true`, then `p` holds.
...
[^5]: A decision procedure is a function that evaluates a proposition and returns `true` if it holds and `false` if it doesn't. We have this automatically for `StepEndsWithCarry`, because the proposition is an equation between two `DfaState` values and we've derived `DecidableEq` for `DfaState` earlier.
